...
class HardwareMonitor:
    def __init__(self):
        ...
        # cpu_percent() ALWAYS returns 0.0 on the very first call
    ...
    def get_stats(self) -> dict:
        """Collect all hardware metrics: CPU, GPU, RAM, power. No sudo required for approx values."""
        stats = {
        ...
        try:
            ...
            stats["e_cpu_pct"] = sum(percpu[:self.e_cores]) / self.e_cores if self.e_cores > 0 else sum(percpu) / len(percpu)
            stats["p_cpu_pct"] = sum(percpu[self.e_cores:]) / self.p_cores if self.p_cores > 0 else stats["e_cpu_pct"]
        ...
        # ASITOP style powermetrics (requires sudo nopasswd configured)
        try:
            import plistlib
