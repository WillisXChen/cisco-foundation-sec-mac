...
<div align="center">
  <img src="screenshots/dev-0.0.1/Apple%20Silicon-Prompt-Grasp-PerfPowMon.png" alt="パフォーマンスモニター" width="800" style="border-radius: 10px; border: 1.2px solid rgba(0, 212, 255, 0.3); box-shadow: 0 0 15px rgba(0, 212, 255, 0.15);">
  <br><br>
...
3. **セキュリティエキスパート**: サイバーセキュリティ領域に特化してファインチューニングされた**Foundation-Sec-8B**により、詳細なシステムおよびセキュリティログ分析を実行します。構造化された簡潔で実用的な技術アドバイスを提供する**エンタープライズ級の簡潔性 (Conciseness)** ロジックを搭載しています。
...
```text
.
├── core/                       # システム核心ロジック (LLM、データベース、ハードウェア、設定)
├── models/                     # GGUFモデルの保存先 (Llama-3 および Foundation-Sec)
├── locales/                    # 多言語翻訳ファイル (.po/.mo)
├── qdrant_storage/             # Qdrantベクトルデータベースの永続的保存先
├── influxdb3_storage/          # メトリクス用の永続的保存先
├── grafana_storage/            # Grafanaダッシュボードの保存先
├── langfuse_db_storage/        # Langfuseトレースデータのローカル保存先
├── public/                     # カスタムブランディング資産 (ロゴ、CSS、テーマ)
├── main.py                     # Chainlitメインアプリケーションのエントリポイント
├── api.py                      # GraphQLおよびカスタムAPIエンドポイント
├── streamlit_app.py            # ASITOP HUD モニタリングインターフェース
├── health_check.py             # システム全体のインフラ健康診断ユーティリティ
├── playbooks.json              # RAG取り込み用の集約されたセキュリティSOP/プレイブック
├── .env                        # 環境変数とシークレット
├── run.sh                      # 高度な実行スクリプト (セットアップ自動化)
└── (その他の設定ファイルやスクリプト)
```
...
サービスが起動すると、ターミナルにChainlitのローカル執行情報が表示されます。通常、ブラウザを開き`http://localhost:8000`にアクセスすることで、セキュリティアシスタントのインターフェースにアクセスできます。

## ⚙️ パフォーマンス最適化 (高度な設定)

システムリソースを制限内 (例: 24GB MacでRAM 50%未満) に抑えるために、`.env` で以下の項目を微調整できます:

*   `N_GPU_LAYERS_LLAMA3`: 一般モデルのGPUオフロードレイヤー数 (-1で全オフロード、0でCPUのみ)。
*   `N_GPU_LAYERS_SEC`: セキュリティモデルのGPUオフロードレイヤー数。
*   `N_CTX_LLAMA3` / `N_CTX_SEC`: コンテキストサイズ (デフォルト 2048)。これを小さくするとRAMを大幅に節約できます。

## 📊 オブザーバビリティとモニタリング

本システムにはエンタープライズグレードの監視ツールが統合されています:

- **Langfuse**: LLM呼び出し、コスト、トークン使用量を追跡します。
- **Arize Phoenix**: RAG応答の自動評価とトレースを実行します。
- **ASITOP HUD**: GPU/CPU/RAM使用率をリアルタイムでフローティング表示します。
- **Grafana**: パフォーマンスの履歴ダッシュボードを表示します。
- **エコシステム状況診断**: `python health_check.py` を実行して、Docker/MLエコシステム全体（Qdrant, InfluxDB, Grafana, Langfuse, Phoenix）の状態を一括確認できます。
